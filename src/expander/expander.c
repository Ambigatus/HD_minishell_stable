...
/*   Updated: 2023/04/12 22:54:02 by ddzuba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	loop_if_dollar_sign(t_global *global, char *str, char **tmp, int j)
{
	...
	while (global->envp[k])
	{
		if (ft_strncmp(str + j + 1, global->envp[k],
				equal_sign(global->envp[k]) - 1) == 0
			&& after_dol_lenght(str, j) - j == (int)equal_sign(global->envp[k]))
		{
			tmp2 = ft_strdup(global->envp[k] + equal_sign(global->envp[k]));
			tmp3 = ft_strjoin(*tmp, tmp2);
			free(*tmp);
			*tmp = tmp3;
			free(tmp2);
			ret = equal_sign(global->envp[k]);
		}
		k++;
	}
	if (ret == 0)
...
char	*detect_dollar_sign(t_global *global, char *str)
{
	int		j;
	char	*tmp;
	char	*tmp2;
...
char	**expander(t_global *global, char **str)
{
	...
	while (str[i] != NULL)
	{
		...
		{
			tmp = detect_dollar_sign(global, str[i]);
			free(str[i]);
			str[i] = tmp;
		}
...
char	*expander_str(t_global *global, char *str)
{
	...
	{
		tmp = detect_dollar_sign(global, str);
		free(str);
		str = tmp;
	}
